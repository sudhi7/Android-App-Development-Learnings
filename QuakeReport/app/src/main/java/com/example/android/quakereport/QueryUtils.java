...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONObject earthquakes_json = new JSONObject(SAMPLE_JSON_RESPONSE);

            JSONArray earthquakes_array = earthquakes_json
                    .getJSONArray("features");

            for (int i=0; i<earthquakes_array.length(); i++) {
                JSONObject earthquake_json = earthquakes_array.getJSONObject(i)
                        .getJSONObject("properties");
                long time_in_unix = earthquake_json.getLong("time");
                Earthquake earthquake = new Earthquake(
                        earthquake_json.getString("place"),
                        earthquake_json.getDouble("mag"),
                        earthquake_json.getInt("time"));
